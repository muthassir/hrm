...
exports.dailySummary = async (req, res, next) => {
  try {
    ...
    const employees = await User.find({ role: 'employee' }).select('_id name department');
    ...
    res.json({ success: true, data: { date, totalEmployees: employees.length, present, absent, lateCount } });
  } catch (err) { next(err); }
};
